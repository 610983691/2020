    ...
    class SentinelNacosDatasource {
        public void init() {
            ...
            ReadableDataSource<String, List<FlowRule>> flowRuleDataSource =
                new NacosDataSource<>(properties, SENTINEL_GROUP, appname + FLOW_RULE_DATA_ID_POSTFIX,
                    source -> JSON.parseObject(source, new TypeReference<List<FlowRule>>() {}));
            FlowRuleManager.register2Property(flowRuleDataSource.getProperty());

            // 注册降级规则
            ReadableDataSource<String, List<DegradeRule>> degradeRuleDataSource =
                new NacosDataSource<>(properties, SENTINEL_GROUP, appname + DEGRADE_RULE_DATA_ID_POSTFIX,
                    source -> JSON.parseObject(source, new TypeReference<List<DegradeRule>>() {}));
            DegradeRuleManager.register2Property(degradeRuleDataSource.getProperty());
            // 注册系统规则
            ReadableDataSource<String, List<SystemRule>> systemRuleDataSource =
                new NacosDataSource<>(properties, SENTINEL_GROUP, appname + SYSTEM_RULE_DATA_ID_POSTFIX,
                    source -> JSON.parseObject(source, new TypeReference<List<SystemRule>>() {}));
            SystemRuleManager.register2Property(systemRuleDataSource.getProperty());

            // 注册授权规则
            ReadableDataSource<String, List<AuthorityRule>> authorityRuleDataSource =
                new NacosDataSource<>(properties, SENTINEL_GROUP, appname + AUTHORITY_RULE_DATA_ID_POSTFIX,
