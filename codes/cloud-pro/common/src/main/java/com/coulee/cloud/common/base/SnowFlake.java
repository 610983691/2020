...
public class SnowFlake {
    ...
    /**
     * 机器ID的值，因为同一个机器上不会部署两个相同的应用。所以同一机器产生相同的主键时，这两个主键也是入不同的业务表。 对日志的ID，这种情况日志的量级不应该是入数据库。一般入ES/mango
     * 等非关系型数据库，利用es他们自身提供的docid来做主键， 而我们的logid只是一个普通字段。（目前应该也达不到这个量级）
     */
    ...
    /**
     * 构造方法,需要配置机器ID。 这个实例化可以采用 如果未配置机器ID：就默认以当前的IP的后两位字节的int值为机器ID
     * 
     * 
     * @param machineId
     */
